...
import json
from util import distance, create_directory
...
def Image_simulator(N_images, image_width, image_height, min_N_fibres, max_N_fibres, delta, l_min, lmin_Analog,
    ...
    create_directory(image_dir_path)
    create_directory(csv_dir_path)
    ...
    for i in range(N_images):
        ...
        for j in range(N):
            ...
            if l_fibre > min_fibre and prob_analog == ['true']:
                ...
                for k in range(n_pair_analog):
                    ...
                    plt.plot((Analog1_x1, Analog1_x2), (Analog1_y1, Analog1_y2), color=color1,
                             linewidth=fiber_width - diff_analog_width)

                    if both_analogs == ['true']:
                        # dessiner cldu sur l'image
                        plt.plot((Analog2_x1, Analog2_x2), (Analog2_y1, Analog2_y2), color=color2,
                                 linewidth=fiber_width - diff_analog_width)
        ...
        fibre.to_csv(csv_dir_path + '/image_' + str(i + 3430) + '_mask',
                     header=['X1', 'Y1', 'X2', 'Y2', 'slop', 'b', 'width', 'N_analogs'], index=False)
...
Image_simulator(N_images, image_width, image_height, min_N_fibres, max_N_fibres, delta, l_min, lmin_Analog,
                lmax_Analog, lmax, Max_analog, dif_analg, min_fibre, output_dir_path, csv_path)
